...
/** Initial values for the core scalar counters/gauges on the metrics root. */
...
/** Initial values for network-quality / reliability scalars and nested state. */
...
/** Initial value for the bandwidth sub-object (excluding the history buffer). */
function buildBandwidthMetrics() {
  return {
    bytesOut: 0,
    bytesIn: 0,
    bytesOutRaw: 0,
...
/** Initial value for the smart-batching sub-object. */
function buildSmartBatchingMetrics() {
  return {
    earlySends: 0,
    timerSends: 0,
    oversizedPackets: 0,
...
function recordError(metrics: Metrics, category: string, message: string): void {
  const counterMap = {
...
function resetMetrics(metrics: Metrics): void {
  Object.assign(metrics, {
    ...buildCoreMetrics(),
    ...buildReliabilityMetrics(),
    remoteNetworkQuality: buildRemoteNetworkQuality()
  });
...
// stalest pointer when still valid, otherwise falls back to a linear scan.
function evictStalestPathIfFull(metrics: Metrics, pathStats: Map<string, PathStat>): void {
...
function trackPathValue(
  ...
  evictStalestPathIfFull(metrics, pathStats);
...
function trackPathStats(metrics: Metrics, delta: Delta, deltaSize: number | null = null): void {
  ...
  for (const update of delta.updates) {
    ...
    for (let i = 0; i < values.length; i++) {
      ...
      trackPathValue(metrics, pathStats, path, bytesPerPath, now);
    }
  }
}
...
function createMetrics(): MetricsApi {
  const metrics: Metrics = {
    ...
    smartBatching: buildSmartBatchingMetrics()
  };
  return {
    metrics,
    recordError: (category: string, message: string) => recordError(metrics, category, message),
    resetMetrics: () => resetMetrics(metrics),
    updateBandwidthRates: (isServerMode: boolean) => updateBandwidthRates(metrics, isServerMode),
    trackPathStats: (delta: Delta, deltaSize: number | null = null) =>
      trackPathStats(metrics, delta, deltaSize),
    formatBytes,
    getTopNPaths: (n: number, uptimeSeconds: number) => getTopNPaths(metrics, n, uptimeSeconds)
  };
}
