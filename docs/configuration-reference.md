...
| `pathPrecision`        | object  | —              | Client-only. Map of path glob to decimal places, e.g. `{ "navigation.speedOverGround": 2 }`. **Lossy:** values are rounded before transmission and the original precision cannot be recovered by the receiver. Use it to cut bandwidth on high-rate numeric paths.                                                                                                                                                                                                                                                                                                                                    |
| `pathThrottle`         | object  | —              | Client-only. Map of path glob to minimum interval in ms, e.g. `{ "navigation.position": 1000 }`. Values arriving faster than the interval are dropped. The highest-leverage bandwidth control for chatty instruments.                                                                                                                                                                                                                                                                                                                                                                                 |
