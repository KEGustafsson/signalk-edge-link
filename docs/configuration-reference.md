...
| Setting              | JSON Key             | Type   | Range          | Description                                                                                                             |
| -------------------- | -------------------- | ------ | -------------- | ----------------------------------------------------------------------------------------------------------------------- |
| Management API Token | `managementApiToken` | string | 16 - 256 chars | Shared secret required by management endpoints protected by `authorizeManagement` (`/instances`, `/bonding`, `/status`) |

**Example (secure management API token):**

```json
{
  "managementApiToken": "replace-with-a-long-random-token",
  "connections": [
    {
      "name": "shore-server",
      "serverType": "server",
