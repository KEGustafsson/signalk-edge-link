...
- [x] `src/metrics-publisher.ts` - MetricsPublisher class (276 lines)
- [x] `__tests__/v2/metrics-publisher.test.js` - 51 comprehensive tests (618 lines)

### Modified Files

- [x] `src/pipeline-v2-client.ts` - Added metrics publishing, RTT/jitter tracking, NAT keepalive
- [x] `src/pipeline-v2-server.ts` - Added metrics publishing, packet loss calculation
- [x] `src/routes.ts` - Added `/network-metrics` endpoint, enhanced `/metrics` with networkQuality
...
### MetricsPublisher (src/metrics-publisher.ts)
