...
function createPipelineV2Server(app, state, metricsApi) {
  ...
  const REMOTE_TELEMETRY_PREFIX = "networking.edgeLink.remote.";
  ...
  function _ingestRemoteTelemetry(deltaMessage) {
    ...
    for (const update of deltaMessage.updates) {
      ...
      for (const entry of update.values) {
        if (!entry || typeof entry.path !== "string" || !entry.path.startsWith(REMOTE_TELEMETRY_PREFIX)) {
          continue;
        }

        switch (entry.path) {
          case "networking.edgeLink.remote.rtt": {
          ...
          case "networking.edgeLink.remote.jitter": {
          ...
          case "networking.edgeLink.remote.packetLoss": {
          ...
          case "networking.edgeLink.remote.retransmissions": {
          ...
          case "networking.edgeLink.remote.queueDepth": {
          ...
          case "networking.edgeLink.remote.retransmitRate": {
          ...
          case "networking.edgeLink.remote.activeLink":
