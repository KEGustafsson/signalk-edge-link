...
function createPipelineV2Server(app, state, metricsApi) {
  ...
  let firstDataSeq = null;
  let highestDataSeq = null;
  ...
  async function receivePacket(packet, secretKey, rinfo) {
    try {
      ...
      if (firstDataSeq === null) {
        firstDataSeq = parsed.sequence >>> 0;
        highestDataSeq = firstDataSeq;
      } else if (isAhead(parsed.sequence >>> 0, highestDataSeq)) {
        highestDataSeq = parsed.sequence >>> 0;
      }
  ...
  function _publishServerMetrics() {
    ...
    // Calculate packet loss using DATA packets only (excludes heartbeats, hellos, duplicates)
    const totalExpected =
      firstDataSeq !== null && highestDataSeq !== null
        ? (((highestDataSeq - firstDataSeq) >>> 0) + 1)
        : 0;
    const totalReceived = metrics.dataPacketsReceived || 0;
    const packetLoss = totalExpected > 0 ?
      Math.max(0, (totalExpected - totalReceived) / totalExpected) : 0;
