...
const { MAX_DECOMPRESSED_SIZE, MAX_DELTAS_PER_PACKET, MAX_CLIENT_SESSIONS, METRICS_PUBLISH_INTERVAL, UDP_RATE_LIMIT_WINDOW, UDP_RATE_LIMIT_MAX_PACKETS } = require("./constants");
    ...
    if (!clientSessions.has(key)) {
      ...
      if (clientSessions.size >= MAX_CLIENT_SESSIONS) {
        ...
        if (oldestKey) {
          ...
          if (evicted) { evicted.sequenceTracker.reset(); }
          clientSessions.delete(oldestKey);
          app.debug(`[v2-server] session evicted (at capacity): ${oldestKey}`);
        }
      }
    ...
    try {
      ...
      if (session) {
        ...
        if (session.rateLimitCount > UDP_RATE_LIMIT_MAX_PACKETS) {
          ...
          app.debug(`[v2-server] rate limited ${session.key}: ${session.rateLimitCount} packets in window`);
          return;
        }
      }
      ...
      if (deltas.length > MAX_DELTAS_PER_PACKET) {
        app.error(`v2 received ${deltas.length} deltas in one packet (limit ${MAX_DELTAS_PER_PACKET}), truncating`);
      }
