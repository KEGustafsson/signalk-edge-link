...
function createInstance(app, options, instanceId, pluginId, onStatusChange) {
  ...
  async function sendDeltaBatch(batch) {
    if (state.pipeline) {
      ...
      await getV1Pipeline().packCrypt(batch, options.secretKey, options.udpAddress, options.udpPort);
    }
  }
  ...
  async function flushDeltaBatch(batchSize = state.deltas.length, retryCount = 0) {
    ...
    try {
      ...
      app.debug(`[${instanceId}] Batch send failed (attempt ${nextRetryCount}/${DELTA_SEND_MAX_RETRIES + 1}): ${err.message}`);
  ...
  function processDelta(delta) {
    ...
    if (state.deltas.length >= MAX_DELTAS_BUFFER_SIZE) {
      ...
      state.droppedDeltaBatches++;
      recordError("sendFailure", `[${instanceId}] Delta buffer overflow, dropped ${dropCount} oldest items`);
    }
  ...
  function stop() {
    ...
    clearTimeout(state.pendingRetry); state.pendingRetry = null;
