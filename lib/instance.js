...
const { loadConfigFile, saveConfigFile, ConfigFileLoadError } = require("./config-io");
...
function createInstance(app, options, instanceId, pluginId, onStatusChange) {
  ...
  function createWatcherWithRecovery(filePath, onChange, name) {
    ...
    function scheduleWatcherRecreate() {
      ...
      watcherObj.recoveryTimer = setTimeout(() => {
        ...
        if (!created) {
          // Keep retrying while the file is temporarily unavailable.
    ...
    function createWatcher() {
      try {
        ...
        watcherObj.watcher = watch(filePath, (eventType) => {
          // Many editors use atomic write (rename + replace), so listen to both.
          if (eventType === "change" || eventType === "rename") {
            app.debug(`[${instanceId}] ${name} file changed`);
            onChange();
            // File rename/replacement can invalidate watch handles.
            if (eventType === "rename") {
              scheduleWatcherRecreate();
            }
          }
  ...
  async function migrateLegacyConfigFiles(instanceDir) {
    ...
    for (const file of legacyFiles) {
      ...
      try {
        ...
        // Use wx flag: create exclusively — fails if target already exists,
        // preventing race conditions with concurrent instance starts.
        await writeFile(target, data, { encoding: "utf-8", flag: "wx" });
        app.debug(`[${instanceId}] Migrated legacy ${file} → instances/default/${file}`);
      } catch (err) {
        // ENOENT: legacy file doesn't exist (nothing to migrate)
        // EEXIST: target already exists (already migrated or created concurrently)
  ...
  async function initializePersistentStorage() {
    ...
    for (const { file, data, name } of defaults) {
      let existing;
      try {
        existing = await loadConfigFile(file);
      } catch (err) {
        if (err instanceof ConfigFileLoadError && err.type === "invalid_json") {
          app.error(`[${instanceId}] invalid JSON config in ${name}: ${err.message}`);
        }
        throw err;
      }
    ...
    if (options.serverType === true || options.serverType === "server") {
      ...
      state.socketUdp.bind(options.udpPort, (err) => {
        if (err) {
          ...
          _setStatus(`Failed to start – ${err.message}`, false);
        }
      });
      ...
      try {
        deltaTimerTimeFile = await loadConfigFile(state.deltaTimerFile);
      } catch (err) {
        if (err instanceof ConfigFileLoadError && err.type === "invalid_json") {
          app.error(`[${instanceId}] invalid JSON config in delta_timer.json: ${err.message}`);
        }
        throw err;
      }
      state.deltaTimerTime = (
