...
// ── File I/O helpers (duplicated here to avoid routes.js dependency) ──────────
...
function createInstance(app, options, instanceId, pluginId, onStatusChange) {
  ...
  function _setStatus(msg) {
    ...
    state.isHealthy = msg ? !msg.toLowerCase().includes("error") && !msg.toLowerCase().includes("fail") : false;
  ...
  function filterOutboundDelta(delta) {
    ...
    for (const update of delta.updates) {
      ...
      const filteredValues = update.values.filter((entry) => {
        ...
        if (path === "networking.edgeLink" || path.startsWith("networking.edgeLink.")) { return false; }
        if (
        ...
        app.subscriptionmanager.subscribe(
          ...
          (delta) => {
            ...
            if (state.deltas.length >= MAX_DELTAS_BUFFER_SIZE) {
              app.error(`[${instanceId}] Delta buffer overflow (${state.deltas.length} items), clearing`);
              state.deltas = [];
            }
    ...
    if (options.serverType === true || options.serverType === "server") {
      ...
      if (useV2) {
        state.monitoring = {
          ...
          alertManager: new AlertManager(appProxy, { thresholds: options.alertThresholds || {} }),
