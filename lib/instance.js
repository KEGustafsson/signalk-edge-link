...
function createInstance(app, options, instanceId, pluginId, onStatusChange) {
  ...
  function filterOutboundDelta(delta) {
    ...
    for (const update of delta.updates) {
      ...
      const filteredValues = update.values.filter((entry) => {
        ...
        if (state.instanceId && path === `networking.modem.${state.instanceId}.rtt`) { return false; }
        ...
        // Only suppress this instance's own notifications; allow other
        // instances' notifications to be forwarded for remote monitoring.
        const ownNotifPrefix = `notifications.signalk-edge-link.${state.instanceId}`;
        if (path === "notifications.signalk-edge-link") { return false; }
        if (path === ownNotifPrefix || path.startsWith(ownNotifPrefix + ".")) { return false; }
        return true;
      });
   ...
   * Filters, buffers, and dispatches deltas to the send pipeline.
   *
