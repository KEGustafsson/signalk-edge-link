...
class PacketBuilder {
  ...
  buildHelloPacket(info = {}) {
    const payload = Buffer.from(JSON.stringify({
      ...
      timestamp: Date.now(),
      capabilities: info.capabilities || ["compression", "encryption", "reliability"]
    }));
  ...
  _buildPacket(type, payload, flags) {
    ...
    // For control packets (non-DATA), append a CRC16 of the payload to detect
    // tampering/corruption.  DATA packets are authenticated by AES-256-GCM so
    // a payload CRC would be redundant.
    if (type !== PacketType.DATA && payload.length > 0) {
      const payloadCrc = Buffer.alloc(2);
      payloadCrc.writeUInt16BE(crc16(payload), 0);
      // Update length field to include the trailing CRC
      const totalPayload = Buffer.concat([payload, payloadCrc]);
      header.writeUInt32BE(totalPayload.length, 9);
      // Recompute header CRC since the length field changed
      const updatedCrc = crc16(header.subarray(0, 13));
      header.writeUInt16BE(updatedCrc, 13);
      return Buffer.concat([header, totalPayload]);
    }
...
class PacketParser {
  ...
  parseHeader(packet) {
    ...
    // For control packets with payload, verify and strip the trailing CRC16
    if (type !== PacketType.DATA && payload.length >= 2) {
