  ...
  async function unpackDecrypt(packet, secretKey) {
    try {
      ...
      if (decompressed.length > MAX_DECOMPRESSED_SIZE) {
        app.error(`Decompressed payload exceeds size limit (${decompressed.length} > ${MAX_DECOMPRESSED_SIZE})`);
        recordError("compression", "Decompressed payload exceeds size limit");
        return;
      }
