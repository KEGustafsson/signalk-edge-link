...
class MetricsPublisher {
  ...
  publish(metrics) {
    ...
    if (metrics.rtt !== undefined) {
      ...
      values.push({ path: "networking.edgeLink.rtt", value: parseFloat(avgRtt.toFixed(1)) });
    }
    ...
    if (metrics.jitter !== undefined) {
      ...
      values.push({ path: "networking.edgeLink.jitter", value: parseFloat(avgJitter.toFixed(1)) });
    }
    ...
    if (metrics.packetLoss !== undefined) {
      ...
      values.push({ path: "networking.edgeLink.packetLoss", value: parseFloat(avgLoss.toFixed(2)) });
    }
    ...
    if (metrics.compressionRatio !== undefined) {
      values.push({
        ...
        value: parseFloat(metrics.compressionRatio.toFixed(0))
      });
    }
