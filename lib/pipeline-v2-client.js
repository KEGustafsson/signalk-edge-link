...
function createPipelineV2Client(app, state, metricsApi) {
  ...
  const retransmitMaxAge = (state.options && state.options.reliability && state.options.reliability.retransmitMaxAge) || 120000;
  const retransmitMinAge = (state.options && state.options.reliability && state.options.reliability.retransmitMinAge) || 10000;
  const retransmitRttMultiplier = (state.options && state.options.reliability && state.options.reliability.retransmitRttMultiplier) || 12;
  const ackIdleDrainAge = (state.options && state.options.reliability && state.options.reliability.ackIdleDrainAge) || 20000;
  const forceDrainAfterAckIdle =
    state.options && state.options.reliability && state.options.reliability.forceDrainAfterAckIdle !== undefined
      ...
      : false;
  const forceDrainAfterMs = (state.options && state.options.reliability && state.options.reliability.forceDrainAfterMs) || 45000;
  const recoveryBurstEnabled =
    state.options && state.options.reliability && state.options.reliability.recoveryBurstEnabled !== undefined
      ? !!state.options.reliability.recoveryBurstEnabled
      : true;
  const recoveryBurstSize = (state.options && state.options.reliability && state.options.reliability.recoveryBurstSize) || 100;
  const recoveryBurstIntervalMs = (state.options && state.options.reliability && state.options.reliability.recoveryBurstIntervalMs) || 200;
  const recoveryAckGapMs = (state.options && state.options.reliability && state.options.reliability.recoveryAckGapMs) || 4000;
   ...
   */
  function receiveACK(packet, rinfo) {
    try {
      ...
      if (entry && (entry.originalTimestamp || entry.timestamp)) {
        rttSample = now - (entry.originalTimestamp || entry.timestamp);
      } else if (lastDataSentAt > 0 && now >= lastDataSentAt) {
        // Fallback path after restart/recovery when exact acked seq is no longer in queue.
        rttSample = now - lastDataSentAt;
      }
    ...
    if (
      ...
      const telemetryDelta = {
        ...
        updates: [{
          ...
          values: [
            { path: "networking.edgeLink.remote.rtt", value: metrics.rtt || 0 },
            { path: "networking.edgeLink.remote.jitter", value: metrics.jitter || 0 },
            { path: "networking.edgeLink.remote.packetLoss", value: packetLoss },
            { path: "networking.edgeLink.remote.retransmissions", value: metrics.retransmissions || 0 },
            { path: "networking.edgeLink.remote.queueDepth", value: retransmitQueue.getSize() },
            { path: "networking.edgeLink.remote.retransmitRate", value: retransmitRate },
            { path: "networking.edgeLink.remote.activeLink", value: bondingManager ? bondingManager.getActiveLinkName() : "primary" }
          ]
        }]
      };
