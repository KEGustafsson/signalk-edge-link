...
const createMetrics = require("./metrics.ts");
const createPipeline = require("./pipeline.ts");
...
const {
  ...
  AlertManager
} = require("./monitoring.ts");
const { PacketCapture, PacketInspector } = require("./packet-capture.ts");
const {
  DEFAULT_DELTA_TIMER,
  PING_TIMEOUT_BUFFER,
...
function createInstance(app, options, instanceId, pluginId, onStatusChange) {
  ...
  async function start() {
    ...
    if (options.serverType === true || options.serverType === "server") {
      ...
      const { loadConfigFile } = require("./config-io.ts");
