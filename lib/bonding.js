...
class BondingManager {
  ...
  _measureLinkHealth(name, link) {
    ...
    for (const [pendingSeq, pendingTs] of link.pendingHeartbeats) {
      if (timestamp - pendingTs > timeout) {
        ...
        if (link.lossSamples.length > BONDING_HEALTH_WINDOW_SIZE) {
          link.lossSamples.shift();
        }
      }
    }
  ...
  _updateLinkMetrics(name, link) {
    ...
    if (link.lossSamples && link.lossSamples.length > 0) {
      const received = link.lossSamples.filter(Boolean).length;
      link.health.loss = Math.max(0, Math.min(1, 1 - (received / link.lossSamples.length)));
