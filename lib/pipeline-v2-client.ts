...
const { encryptBinary } = require("./crypto.ts");
const { encodeDelta } = require("./pathDictionary.ts");
const {
  deltaBuffer,
  compressPayload,
  udpSendAsync: _udpSendAsyncShared
} = require("./pipeline-utils.ts");
const { PacketBuilder, PacketParser, PacketType } = require("./packet.ts");
const { RetransmitQueue } = require("./retransmit-queue.ts");
const { MetricsPublisher } = require("./metrics-publisher.ts");
const { CongestionControl } = require("./congestion.ts");
const { BondingManager } = require("./bonding.ts");
const {
    ...
    try {
      ...
      if (rttSample !== null) {
        ...
        if (rttSamples.length >= 2) {
          ...
          const variance =
            samples.reduce((sum, s) => sum + Math.pow(s - avg, 2), 0) / samples.length;
