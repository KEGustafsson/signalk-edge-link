...
function register(router, ctx) {
  ...
  async function restartWithConnections(res, connections, successStatus = 200) {
    ...
    const sanitizedConnections = connections.map((connection) => sanitizeConnectionConfig(connection));

    await pluginRef._restartPlugin({ connections: sanitizedConnections });
    pluginRef._currentOptions = { connections: sanitizedConnections };
