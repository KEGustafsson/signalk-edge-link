...
function register(router, ctx) {
  const {
    ...
    buildFullMetricsResponse, pluginRef, authorizeManagement
  } = ctx;
  ...
  function _deriveInstanceIds(connections) {
    const used = new Set();
    return connections.map((cfg) => {
      const base = _slugify(cfg.name || "connection");
      if (!used.has(base)) {
        used.add(base);
        return base;
      }
  ...
  function findConnectionIndexByInstanceId(connections, instanceId) {
    const ids = _deriveInstanceIds(connections);
  ...
  function validateUniqueServerPorts(connections) {
    const serverPorts = connections
      .filter((c) => c && (c.serverType === "server" || c.serverType === true))
      .map((c) => Number(c.udpPort))
      .filter((p) => Number.isInteger(p));

    const duplicates = serverPorts.filter((p, idx) => serverPorts.indexOf(p) !== idx);
  ...
  function validateNewConnection(connection) {
    if (!connection || typeof connection !== "object" || Array.isArray(connection)) {
      return "Request body must be a JSON object";
    }

    const requiredKeys = ["name", "serverType", "udpPort", "secretKey"];
    ...
    if (connection.serverType !== "server" && connection.serverType !== "client") {
      return "serverType must be 'server' or 'client'";
    }

    const udpPort = Number(connection.udpPort);
    if (!Number.isInteger(udpPort) || udpPort < 1024 || udpPort > 65535) {
      return "udpPort must be an integer between 1024 and 65535";
    }

    if (typeof connection.secretKey !== "string" || connection.secretKey.length !== 32) {
      return "secretKey must be exactly 32 characters";
    }
  ...
  async function restartWithConnections(res, connections, successStatus = 200) {
    ...
    await pluginRef._restartPlugin({ connections });
    pluginRef._currentOptions = { connections };
    return res.status(successStatus).json({ success: true });
  }
  function validateConfigBody(filename, body) {
  ...
  router.get("/instances", rateLimitMiddleware, (req, res) => {
    try {
      if (!authorizeManagement(req, res, "instances.list")) { return; }
      const all = instanceRegistry.getAll();
      ...
      const mapped = all.map((b) => ({
        ...
        currentLink: (b.state.pipeline && b.state.pipeline.getBondingManager)
          ? (b.state.pipeline.getBondingManager() && b.state.pipeline.getBondingManager().getState().activeLink) || "primary"
          : "primary",
        metrics: {
      ...
      const filtered = stateFilter
        ? mapped.filter((item) => String(item.state || "").toLowerCase() === stateFilter.toLowerCase())
        : mapped;
  ...
  router.get("/instances/:id", rateLimitMiddleware, (req, res) => {
    try {
      if (!authorizeManagement(req, res, "instances.show")) { return; }
  ...
  router.post("/instances", rateLimitMiddleware, requireJson, async (req, res) => {
    try {
      if (!authorizeManagement(req, res, "instances.create")) { return; }
      const body = req.body || {};
      const validationError = validateNewConnection(body);
      if (validationError) {
        return res.status(400).json({ error: validationError });
      }

      const connections = getCurrentConnectionsConfig();
      connections.push({ ...body, udpPort: Number(body.udpPort) });
  ...
  router.put("/instances/:id", rateLimitMiddleware, requireJson, async (req, res) => {
    try {
      if (!authorizeManagement(req, res, "instances.update")) { return; }
      const bundle = getBundleById(req.params.id);
      if (!bundle) {
        return res.status(404).json({ error: `Instance '${req.params.id}' not found` });
      }
      ...
      if (idx === -1) {
        return res.status(404).json({ error: `Configuration for instance '${req.params.id}' not found` });
      }
      ...
      for (const key of patchKeys) {
        if (immutable.has(key)) {
          return res.status(400).json({ error: `Field '${key}' is not updatable via /instances/:id` });
        }
        if (!mutableAllowed.has(key)) {
          return res.status(400).json({ error: `Field '${key}' is not supported for /instances/:id updates` });
        }
      }
  ...
  router.delete("/instances/:id", rateLimitMiddleware, async (req, res) => {
    try {
      if (!authorizeManagement(req, res, "instances.delete")) { return; }
      const bundle = getBundleById(req.params.id);
      if (!bundle) {
        return res.status(404).json({ error: `Instance '${req.params.id}' not found` });
      }
  ...
  router.get("/connections/:id/config/:filename", rateLimitMiddleware, async (req, res) => {
    ...
    try {
      ...
      if (err && err.name === "ConfigFileLoadError" && err.type === "invalid_json") {
        app.error(`Invalid JSON config in ${req.params.filename} for connection ${req.params.id}: ${err.message}`);
  ...
  router.post("/connections/:id/config/:filename", rateLimitMiddleware, requireJson, async (req, res) => {
    ...
    try {
      const filePath = getConfigFilePath(state, req.params.filename);
      if (!filePath) {return res.status(400).json({ error: "Invalid filename" });}
      const validationError = validateConfigBody(req.params.filename, req.body);
