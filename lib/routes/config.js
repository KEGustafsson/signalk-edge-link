...
function register(router, ctx) {
  ...
  function validateOneConnection(c, prefix) {
    ...
    if (c.serverType === true) {
      c.serverType = "server";
    }
    if (c.serverType === false) {
      c.serverType = "client";
    }
    ...
    if (c.alertThresholds !== undefined) {
      if (
        !c.alertThresholds ||
        typeof c.alertThresholds !== "object" ||
        Array.isArray(c.alertThresholds)
      ) {
    ...
    if (c.reliability !== undefined) {
      ...
      if (
        rel.forceDrainAfterAckIdle !== undefined &&
        typeof rel.forceDrainAfterAckIdle !== "boolean"
      ) {
      ...
      if (
        rel.retransmitMinAge !== undefined &&
        rel.retransmitMaxAge !== undefined &&
    ...
    if (c.congestionControl !== undefined) {
      if (
        !c.congestionControl ||
        typeof c.congestionControl !== "object" ||
        Array.isArray(c.congestionControl)
      ) {
  ...
  function validateManagementApiToken(token) {
    if (token === undefined) {
      return null;
    }
    if (typeof token !== "string") {
      return "managementApiToken must be a string";
    }
    const trimmed = token.trim();
    if (!trimmed) {
      return "managementApiToken must be a non-empty string";
    }
    if (trimmed.length < 16 || trimmed.length > 256) {
      return "managementApiToken must be between 16 and 256 characters";
    }
    return null;
  }
  ...
  router.post("/plugin-config", rateLimitMiddleware, requireJson, (req, res) => {
    try {
      ...
      const tokenError = validateManagementApiToken(req.body.managementApiToken);
      if (tokenError) {
      ...
      if (req.body.managementApiToken !== undefined) {
        finalConfig.managementApiToken = req.body.managementApiToken.trim();
      }
  ...
  const clientModeMiddleware = (req, res, next) => {
    ...
    if (!b.state.deltaTimerFile || !b.state.subscriptionFile) {
      ...
      try {
        ...
        if (filename === "delta_timer.json") {
          if (
            ...
            return res
              .status(400)
              .json({ error: "deltaTimer must be a number between 100 and 10000" });
          }
