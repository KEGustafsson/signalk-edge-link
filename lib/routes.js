  ...
  function registerWithRouter(router) {
    ...
    router.get("/prometheus", rateLimitMiddleware, (req, res) => {
      try {
        ...
        const text = formatPrometheusMetrics(prometheusMetrics, state, extra);
    ...
    router.post("/plugin-config", rateLimitMiddleware, requireJson, (req, res) => {
      try {
        ...
        const isValidPort = (v, min = 1) => Number.isInteger(v) && v >= min && v <= 65535;
        ...
        if (newConfig.serverType === true) {
          newConfig.serverType = "server";
        } else if (newConfig.serverType === false) {
          newConfig.serverType = "client";
        }
        if (newConfig.serverType !== "server" && newConfig.serverType !== "client") {
          return res.status(400).json({ success: false, error: "serverType must be 'server' or 'client'" });
        }
        if (!isValidPort(newConfig.udpPort, 1024)) {
          return res.status(400).json({ success: false, error: "Valid udpPort (1024-65535) is required" });
        }
        try {
          validateSecretKey(newConfig.secretKey);
        } catch (error) {
          return res.status(400).json({ success: false, error: error.message });
        }
        if (newConfig.protocolVersion !== undefined && newConfig.protocolVersion !== 1 && newConfig.protocolVersion !== 2) {
          return res.status(400).json({ success: false, error: "protocolVersion must be 1 or 2" });
        }
        if (newConfig.useMsgpack !== undefined && typeof newConfig.useMsgpack !== "boolean") {
          return res.status(400).json({ success: false, error: "useMsgpack must be a boolean" });
        }
        if (newConfig.usePathDictionary !== undefined && typeof newConfig.usePathDictionary !== "boolean") {
          return res.status(400).json({ success: false, error: "usePathDictionary must be a boolean" });
        }

        if (newConfig.serverType === "client") {
          if (!newConfig.udpAddress || typeof newConfig.udpAddress !== "string") {
            return res.status(400).json({ success: false, error: "udpAddress is required in client mode" });
          }
          if (!newConfig.testAddress || typeof newConfig.testAddress !== "string") {
            return res.status(400).json({ success: false, error: "testAddress is required in client mode" });
          }
          if (
            !isValidPort(newConfig.testPort, 1)
          ) {
        ...
        // Validate optional alert thresholds
        if (newConfig.alertThresholds !== undefined) {
          if (
            !newConfig.alertThresholds ||
            typeof newConfig.alertThresholds !== "object" ||
            Array.isArray(newConfig.alertThresholds)
          ) {
            return res.status(400).json({ success: false, error: "alertThresholds must be an object" });
          }

          const validAlertMetrics = ["rtt", "packetLoss", "retransmitRate", "jitter", "queueDepth"];
          for (const [metric, threshold] of Object.entries(newConfig.alertThresholds)) {
            if (!validAlertMetrics.includes(metric)) {
              return res.status(400).json({ success: false, error: `Invalid alertThresholds metric: ${metric}` });
            }
            if (!threshold || typeof threshold !== "object" || Array.isArray(threshold)) {
              return res.status(400).json({ success: false, error: `alertThresholds.${metric} must be an object` });
            }
            if (threshold.warning !== undefined && !isFiniteNumber(threshold.warning)) {
              return res.status(400).json({ success: false, error: `alertThresholds.${metric}.warning must be a finite number` });
            }
            if (threshold.critical !== undefined && !isFiniteNumber(threshold.critical)) {
              return res.status(400).json({ success: false, error: `alertThresholds.${metric}.critical must be a finite number` });
            }
            if (
              ...
              return res.status(400).json({
                success: false,
                error: `alertThresholds.${metric}.warning must be less than or equal to critical`
              });
            }
          }
        }
        ...
        // Validate optional bonding settings
        if (newConfig.bonding !== undefined) {
          if (!newConfig.bonding || typeof newConfig.bonding !== "object" || Array.isArray(newConfig.bonding)) {
            return res.status(400).json({ success: false, error: "bonding must be an object" });
          }
          const bonding = newConfig.bonding;
          if (bonding.enabled !== undefined && typeof bonding.enabled !== "boolean") {
            return res.status(400).json({ success: false, error: "bonding.enabled must be a boolean" });
          }
          if (bonding.mode !== undefined && bonding.mode !== "main-backup") {
            return res.status(400).json({ success: false, error: "bonding.mode must be 'main-backup'" });
          }
          for (const linkKey of ["primary", "backup"]) {
            if (bonding[linkKey] !== undefined) {
              const link = bonding[linkKey];
              if (!link || typeof link !== "object" || Array.isArray(link)) {
                return res.status(400).json({ success: false, error: `bonding.${linkKey} must be an object` });
              }
              if (link.address !== undefined && typeof link.address !== "string") {
                return res.status(400).json({ success: false, error: `bonding.${linkKey}.address must be a string` });
              }
              if (link.port !== undefined && !isValidPort(link.port, 1024)) {
                return res.status(400).json({
                  success: false,
                  error: `bonding.${linkKey}.port must be a number between 1024 and 65535`
                });
              }
              if (link.interface !== undefined && typeof link.interface !== "string") {
                return res.status(400).json({ success: false, error: `bonding.${linkKey}.interface must be a string` });
              }
            }
          }
          if (bonding.failover !== undefined) {
            if (!bonding.failover || typeof bonding.failover !== "object" || Array.isArray(bonding.failover)) {
              return res.status(400).json({ success: false, error: "bonding.failover must be an object" });
            }
            const failoverChecks = [
              ["rttThreshold", 100, 5000, "bonding.failover.rttThreshold"],
              ["lossThreshold", 0.01, 0.5, "bonding.failover.lossThreshold"],
              ["healthCheckInterval", 500, 10000, "bonding.failover.healthCheckInterval"],
              ["failbackDelay", 5000, 300000, "bonding.failover.failbackDelay"],
              ["heartbeatTimeout", 1000, 30000, "bonding.failover.heartbeatTimeout"]
            ];
            for (const [key, min, max, label] of failoverChecks) {
              const err = validateNumberRange(bonding.failover, key, min, max, label);
              if (err) {
                return res.status(400).json({ success: false, error: err });
              }
            }
    ...
    const clientModeMiddleware = (req, res, next) => {
      const b = getFirstBundle();
      if (!b || b.state.isServerMode) return res.status(404).json({ error: "Not available in server mode" });
    ...
    // ── Multi-instance: per-connection metrics ────────────────────────────────
    router.get("/connections/:id/metrics", rateLimitMiddleware, (req, res) => {
      ...
      const { metrics, updateBandwidthRates, formatBytes, getTopNPaths } = bundle.metricsApi;
      updateBandwidthRates(state.isServerMode);
      // Delegate to existing helper that builds the full metrics response object
      // (inline the same logic from the root /metrics handler)
      res.json({ instanceId: req.params.id, mode: state.isServerMode ? "server" : "client",
        deltasSent: metrics.deltasSent, deltasReceived: metrics.deltasReceived,
        readyToSend: state.readyToSend, bandwidth: metrics.bandwidth });
    });
