...
const { BROTLI_QUALITY_HIGH, UDP_RETRY_MAX, UDP_RETRY_DELAY } = require("./constants");
    ...
    socket.send(message, port, host, async (error) => {
      if (error) {
        if (retryCount < UDP_RETRY_MAX && (error.code === "EAGAIN" || error.code === "ENOBUFS")) {
          if (callbacks.onRetry) {callbacks.onRetry(retryCount + 1, error);}
          ...
          if (callbacks.onError) {callbacks.onError(error, retryCount);}
          reject(error);
        }
      } else {
        resolve();
