...
const MAX_DELTAS_PER_PACKET = 500; // max deltas to process from a single received packet
...
const BROTLI_QUALITY_HIGH = 6; // Balanced quality: ~90% of max compression at ~10% of the CPU cost
...
const BONDING_LOSS_THRESHOLD = 0.1; // Packet loss threshold for failover (10%)
...
const MAX_CLIENT_SESSIONS = 100; // Max concurrent client sessions per server instance
const UDP_RATE_LIMIT_WINDOW = 1000; // Per-client UDP rate limit window (ms)
const UDP_RATE_LIMIT_MAX_PACKETS = 200; // Max DATA packets per client per window

// Decompression safety
const MAX_DECOMPRESSED_SIZE = 10 * 1024 * 1024; // 10 MB - reject decompression bombs

// Metrics
const METRICS_PUBLISH_INTERVAL = 1000; // Interval (ms) for publishing metrics to Signal K
const BANDWIDTH_HISTORY_MAX = 60; // Keep 60 data points (5 minutes at 5s intervals)
const PATH_STATS_MAX_SIZE = 500; // Max tracked paths in pathStats Map (prevent unbounded growth)

// Enhanced monitoring
const MONITORING_HEATMAP_BUCKETS = 60; // Number of time buckets for packet loss heatmap
const MONITORING_HEATMAP_BUCKET_DURATION = 5000; // Duration of each bucket (5 seconds)
const MONITORING_RETRANSMIT_HISTORY_SIZE = 120; // Retransmission rate history entries (10 minutes at 5s)
const MONITORING_PATH_LATENCY_WINDOW = 50; // Latency samples per path
const MONITORING_ALERT_COOLDOWN = 60000; // Alert cooldown period (1 minute)
const PACKET_CAPTURE_MAX_PACKETS = 1000; // Max packets in capture buffer
const PACKET_INSPECTOR_MAX_CLIENTS = 5; // Max concurrent WebSocket inspector clients
