...
- Prefer existing patterns in `src/index.ts`, `src/app/connection.ts`, `src/routes.ts`, route modules, pipeline modules, and shared schema files.
...
| ------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------- |
| Plugin lifecycle and instance orchestration | `src/index.ts`, `src/app/connection.ts`, `src/app/connection/*`, `src/app/connection-manager.ts`, `src/app/lifecycle.ts`, `src/transport/pipeline/factory.ts`                                                                                    | Architecture, Backend, Test                   |
| v1 transport                                | `src/transport/pipeline/v1.ts`, `src/transport/pipeline/v1-helpers.ts`, `src/codec/crypto.ts`, `src/ping-monitor.d.ts`                                                                                                                           | Protocol, Security, Test                      |
| v2/v3 reliable transport                    | `src/transport/pipeline/reliable-client{,/*}.ts`, `src/transport/pipeline/reliable-server{,/*}.ts`, `src/codec/packet-codec.ts`, `src/codec/packet/*`, `src/transport/reliability/*`, `src/transport/congestion.ts`, `src/transport/bonding*.ts` | Protocol, Security, Test                      |
| Delta processing                            | `src/codec/delta-sanitizer{,/*}.ts`, `src/codec/path-dictionary*.ts`, `src/codec/compact-delta.ts`, `src/codec/value-dedup.ts`, `src/codec/metadata-codec.ts`, `src/codec/metadata/*`, `src/codec/source-*.ts`, `src/codec/values-snapshot.ts`   | Backend, Protocol, Test                       |
| Management routes                           | `src/routes.ts`, `src/routes/*.ts`                                                                                                                                                                                                               | Backend, Security, Operations, Test           |
| Metrics and monitoring                      | `src/domain/metrics/registry.ts`, `src/domain/metrics/prometheus.ts`, `src/domain/monitoring/*`, `src/transport/metrics/publisher.ts`                                                                                                            | Operations, Backend, Test                     |
| Configuration IO and migration              | `src/foundation/config-io.ts`, `src/foundation/config-reload.ts`, `src/app/config/*`, `src/connection-config.ts`, `src/scripts/migrate-config.ts`                                                                                                | Schema, Backend, Test, Docs                   |
| Shared schema                               | `src/shared/connection-schema.ts`, `src/foundation/crypto-constants.ts`, `src/app/config/schema.ts`                                                                                                                                              | Schema, Web UI, Backend                       |
