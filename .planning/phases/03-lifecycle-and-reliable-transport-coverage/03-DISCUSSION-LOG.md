...
**Selected:** Targeted regression seams first — Phase 3 is a coverage phase. Planning should protect meaningful failures rather than chase line counts.
...
**Selected:** Stop/recovery/watcher resource cleanup — use fake timers and mocked sockets/watchers where possible.
...
**Selected:** ACK/NAK and sequence recovery — bonding is already relatively well covered; pipeline v2 client/server files are the priority risk.
...
**Selected:** Cover existing metadata/source restart behavior — keep this to regression coverage for current behavior, not a protocol expansion.
...
**Selected:** Minimal helper extraction only — keep source changes behavior-preserving unless a regression test proves a real bug.
